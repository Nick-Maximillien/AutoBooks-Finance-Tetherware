...
const DJANGO_API_URL = process.env.NEXT_PUBLIC_DJANGO_API_URL || 'https://autobooks-backend-571147915643.us-central1.run.app';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signIn = async (username: string, password: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${DJANGO_API_URL}/token/`, {
        method: 'POST',
  ...
  const signUp = async (userData: { username: string; email: string; password: string; [key: string]: any }) => {
    ...
    try {
      ...
      const res = await fetch(`${DJANGO_API_URL}/signup/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
